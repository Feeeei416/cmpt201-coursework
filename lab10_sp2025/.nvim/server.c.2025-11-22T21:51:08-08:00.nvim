...
1. How is the client sending data to the server? What protocol?
  The client use TCP: socket() -> connect()
  -> write() which sends the data to the server
2. What data is the client sending to the server?
  Since
  static const char *messages[NUM_MSG]
  = {"Hello", "Apple", "Car", "Green","Dog"};
  and inside the loop
  strncpy(buf, messages[i], BUF_SIZE);
  write(sfd, buf, BUF_SIZE);
  So the client sends five test message ("Hello", "Apple", "Car", "Green",
"Dog"). Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
  The argument passed to run_acceptor is a pointer to this struct acceptor_args
that is in main().
2. How are received messages stored?
  The client thread reads a message from its socket into msg_buf.
  Then it allocates a new struct list_node and node->data.
  And we copy the message from msg_buf into node->data.
  Finally, we use the function add_to_list to add new_node to the list.
3. What does `main()` do with the received messages?
  It sets up the list and starts the acceptor_thread.
  Then, main() is supposed to wait until list_handle.count
  reaches MAX_CLIENTS * NUM_MSG_PER_CLIENT.
  It stops the acceptor and checks if the expected number of messages were
received. Finally, it calls collect_all to process and free all message.
4. How are threads used in this sample code?
  The acceptor thread listens for new client connections and creates a separate
client thread for each one. Each client thread reads messages from its client
and stores them in the shared list, while the main thread coordinates startup
and shutdown. Explain the use of non-blocking sockets in this lab.
- How are sockets made non-blocking?
  The sockets are made non-blocking by calling set_non_blocking(fd),
  which uses the fcntl(fd, F_GETFL, 0) and then fcntl(fd, F_SETFL, flags |
O_NONBLOCK).
- What sockets are made non-blocking?
  The server's listening socket sfd in run_acceptor and each client connection
socket sfd in run_client are both set to non-blocking
- Why are these sockets made non-blocking? What purpose does it serve?
  This lets the loop in run_acceptor() call accept() and client threads call
read() without getting stuck waiting.
*/
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
